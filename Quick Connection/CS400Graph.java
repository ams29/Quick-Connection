...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path source) {
            ...
            this.dataSequence = new LinkedList<>();
            for (T d : source.dataSequence) {
                this.dataSequence.add(d);
            }
            this.end = source.end;
        }
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
         ...
         PriorityQueue<Path> visitedPath = new PriorityQueue<>();
         PriorityQueue<T> visitedVertex = new PriorityQueue<>();
         Path startPath = new Path(vertices.get(start));
         pq.add(startPath);
         while (!pq.isEmpty()) {
             Path minDistancePath = pq.peek();
             for (Path p : pq) { // find the min distance path in queue
                 if (p.distance < minDistancePath.distance) {
                     minDistancePath = p;
                 }
             }
             visitedVertex.add(minDistancePath.end.data);

             for (Edge e : minDistancePath.end.edgesLeaving) { // add path
                if (visitedVertex.contains(e.target.data)) { // do nothing if the target is visited
                }else {
                    Path addPath = new Path(minDistancePath);
                    addPath.extend(e);
                    Path containedPath = containsPath(pq, addPath.start.data, addPath.end.data);
                    if (containedPath == null) { // if pq doesn't contain path
                     pq.add(addPath);
                    }else {
                        if (addPath.distance < containedPath.distance) { // contains path but distance is greater
                            pq.remove(containedPath);
                            pq.add(addPath);
                        }
                    }
                }
             ...
             visitedPath.add(minDistancePath);
             pq.remove(minDistancePath);
         }

         Path returnPath = new Path(vertices.get(start));
         for (Path p : visitedPath) { // loop the visited queue to find the path
             if ((p.start.data == start) && (p.end.data == end)) {
                 returnPath = p;
             }
         }
        if (returnPath == null) {
    ...
    /**
     * return path contains the path, otherwise, return null
     * @param pq queue
     * @param start start vertex data
     * @param end end vertex data
     * @return return path contains the path, otherwise, return null
     */
    public Path containsPath(PriorityQueue<Path> pq, T start, T end) {
        for (Path p : pq) {
            if ((p.start.data == start) && (p.end.data == end)) {
                return p;
            }
